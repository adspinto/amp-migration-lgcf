...
https://latingrammyculturalfoundation.org/content/donate-page
https://latingrammyculturalfoundation.org/content/page
https://latingrammyculturalfoundation.org/content/colors
https://latingrammyculturalfoundation.org/content/news-article
https://latingrammyculturalfoundation.org/slot/localized-page
https://latingrammyculturalfoundation.org/content/landing-page
https://latingrammyculturalfoundation.org/slot/home-page
`;
const t = [`https://therecordingacademy.com/partial/legacy-vocabulary`];
const s = schemaIds.split("https://schema-examples.com/media");

// without doing this we can't 
// const list = s[0].replace(/(\r\n|\n|\r)/gm, "").split("https://").map(item => `https://${item}`).filter(item => item !== "https://");
// const list2 = s[1].replace(/(\r\n|\n|\r)/gm, "").split("https://").map(item => `https://${item}`).filter(item => item !== "https://");
const listImport = s[1].replace(/(\r\n|\n|\r)/gm, "").split("https://").map(item => `https://${item}`).filter(item => item !== "https://" && !item.includes("partial"));
// console.log(list)

// console.log(listImport)
// exportCLI({ schemaIds: listImport, client: "lradev", toExport: "content-type", name: "exported-content" })
...
importCLI({ client: "lraprod", toImport: "content-type", name: "exported-content" })
